...
StatLogger::StatLogger(int game_id, int32_t zone_id, int32_t svr_id, int32_t site_id, int isgame, bool writeToTongji , bool writeToTms)
{
	...
	// 创建保存tms数据的目录
	string path2 = "/opt/taomee/stat/tmsdata";
	assert(stat_makedir(path2 + "/inbox"));
	...
	m_basic_fds1[m_appid] = -1;
	m_basic_fds2[m_appid] = -1;
	m_basic_tss1[m_appid] = 0;
	m_basic_tss2[m_appid] = 0;
...
//	m_path[path.size()] = '\0';
//
//	string path2 = "/opt/taomee/stat/tmsdata";//默认统计数据一律写入该目录下
//	assert(stat_makedir(path2 + "/inbox"));
//	path2.copy(m_path2, path2.size());
//	m_path2[path2.size()] = '\0';
//
...
StatLogger* StatLogger::set(int game_id, int32_t zone_id, int32_t svr_id, int32_t site_id, int isgame,bool writeToTongji,bool writeToTms)
{
    	m_appid = game_id;
	m_siteid = site_id;
	m_zoneid = zone_id;
	...
	if(m_basic_fds1.count(m_appid)==0){
		m_basic_fds1[m_appid] = -1;
	}
	if(m_basic_fds2.count(m_appid)==0){
		m_basic_fds2[m_appid] = -1;
	}
	if(m_basic_tss1.count(m_appid)==0){
		m_basic_tss1[m_appid] = 0;
	}
	if(m_basic_tss2.count(m_appid)==0){
		m_basic_tss2[m_appid] = 0;
	}
 ...
 * @param acct_id
 */
void StatLogger::reg_account_system(int acct_gid, uint32_t mimi_id, std::string tad, uint32_t cli_ip,GameType game_type, RegType acct_type, std::string acct_id)
{
						  ...
						  string browser, string device, string os, string resolution, string network, string isp)
{
	...
	ostringstream oss2;
	// 帐号新增
	set_basic_info(oss, "_newac_", "_newac_", ts, acct_id);
	set_basic_info2(oss2, 2, ts, acct_id);
	set_device_info(oss, op, ads_id, browser, device, os, resolution, network, isp);
	...
	if (player_id.size()) {
		...
		set_basic_info2(oss2, 3, ts, acct_id, player_id);
		oss << '\n';
		oss2 << '\n';
	}
	// 职业新增
	if (race.size()) {
		...
		set_basic_info2(oss2, 40, ts, acct_id);
		oss << '\n';
		oss2 << "\t_race_=" << race << '\n';
	}
						...
						string ads_id, string browser, string device, string os, string resolution, string network, string isp)
{
	...
	set_basic_info2(oss2, 1, ts, acct_id);
	set_device_info(oss, op, ads_id, browser, device, os, resolution, network, isp);
	set_device_info(oss2, op, ads_id, browser, device, os, resolution, network, isp);
						...
						string ads_id, string zone, string browser, string device, string os, string resolution, string network, string isp)
{
   ...
   if(lv != 0)
   {
	...
	set_basic_info2(oss2, 5 , ts, acct_id);//设置tms新的日志格式
	oss << "\t_vip_=" << isvip << "\t_lv_=" << lv;
	oss2 << "\t_vip_=" << isvip << "\t_lv_=" << lv;
	set_device_info(oss, op, ads_id, browser, device, os, resolution, network, isp);
	set_device_info(oss2, op, ads_id, browser, device, os, resolution, network, isp);
	...
	if (player_id.size()) {
		...
		set_basic_info2(oss2, 6, ts, acct_id, player_id);
		oss << '\n';
		oss2 << '\n';
	}
	// 职业登录
...
void StatLogger::logout(string acct_id, bool isvip, int lv, int oltime)
{
	...
	set_basic_info2(oss2, 8, ts, acct_id);
...
void StatLogger::level_up(string acct_id, string race, int lv)
{
	...
	set_basic_info2(oss2, 10, ts, acct_id);
	oss << "\t_lv_=" << lv << "\t_op_=set_distr:_lv_" << '\n';
	oss2 << "\t_lv_=" << lv << "\t_op_=set_distr:_lv_" << '\n';
	// 职业等级变化
	if (race.size()) {
		stat_trim_underscore(race);
		MY_ASSERT(is_valid_race(race));
		set_basic_info(oss, "_racelvup_", race, ts, acct_id);
		set_basic_info2(oss2, 11, ts, acct_id);
		oss << "\t_lv_=" << lv << "\t_op_=set_distr:_lv_" << '\n';
...
void StatLogger::use_golds(std::string acct_id, bool is_vip, std::string reason, float amt, int lv)
{
	...
	oss2 << "\t_golds_=" << (m_need_multi?amt*100:amt) << "\t_op_=sum:_golds_\n";

	set_basic_info(oss, "_usegold_", reason, ts, acct_id);
	set_basic_info2(oss2, 75, ts, acct_id);
...
void StatLogger::do_obtain_golds(const string& acct_id, const string& reason, int amt)
{
	...
	if(reason == "_systemsend_"){
		set_basic_info2(oss2, 17, ts, acct_id);
	}
	if(reason == "_userbuy_"){
		set_basic_info2(oss2, 35, ts, acct_id);
	}
	oss << "\t_golds_=" << amt << "\t_op_=sum:_golds_\n";
	oss2 << "\t_golds_=" << amt << "\t_op_=sum:_golds_\n";
...
void StatLogger::write_basic_log(const std::string& s, time_t ts)
{
	...
	if ((m_basic_fds1.count(m_appid)==0)||(m_basic_tss1.count(m_appid)==0)||(ts < m_basic_tss1[m_appid]) || ((ts - m_basic_tss1[m_appid]) > 19) || ((m_basic_tss1[m_appid] + 8*60*60)/86400 != (ts + 8*60*60)/86400))
   	{
		close(m_basic_fds1[m_appid]);
		int basic_ts1 = ts - (ts % 20);
		m_basic_tss1[m_appid]=basic_ts1;
		ostringstream oss;
		if(m_isgame)
		{//游戏后台络的日志
			oss << m_path << "/inbox/" << m_appid << "_game_basic_" << m_basic_tss1[m_appid];
		}
		else
		{//平台组络的日志
			oss << m_path << "/inbox/" << m_appid << "_account_basic_" << m_basic_tss1[m_appid];
		}
		int basic_fd1 = open(oss.str().c_str(), O_WRONLY | O_APPEND | O_CREAT, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
		m_basic_fds1[m_appid]=basic_fd1;
		if (basic_fd1 == -1) {
			errlog(string("[write_basic_log]open(): ") + strerror(errno));
		}
		fchmod(basic_fd1, 0777);
		m_chksum1 = m_chksum2 = calc_checksum();
	}

	if (write(m_basic_fds1[m_appid], s.c_str(), s.size()) == -1) {
...
void StatLogger::write_basic_log2(const std::string& s, time_t ts)
{
	if(m_inited == 0)
	{
		errlog(string("object not inited"));
	...
	// 每20秒时间切换文件，跨天也切换文件
	if ((m_basic_fds2.count(m_appid)==0)||(m_basic_tss2.count(m_appid)==0)||(ts < m_basic_tss2[m_appid]) || ((ts - m_basic_tss2[m_appid]) > 19) || ((m_basic_tss2[m_appid] + 8*60*60)/86400 != (ts + 8*60*60)/86400))
   	{
		close(m_basic_fds2[m_appid]);
		int basic_ts2 = ts - (ts % 20);
		m_basic_tss2[m_appid]=basic_ts2;
		ostringstream oss;
		if(m_isgame)
		{//游戏后台络的日志
			oss << m_path2 << "/inbox/" << m_appid << "_game_basic_" << m_basic_tss2[m_appid];
		}
		else
		{//平台组络的日志
			oss << m_path2 << "/inbox/" << m_appid << "_account_basic_" << m_basic_tss2[m_appid];
		}
		int basic_fd2 = open(oss.str().c_str(), O_WRONLY | O_APPEND | O_CREAT, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
		m_basic_fds2[m_appid]=basic_fd2;
		if (basic_fd2 == -1) {
			errlog(string("[write_basic_log]open(): ") + strerror(errno));
		}
		fchmod(basic_fd2, 0777);
		m_chksum1 = m_chksum2 = calc_checksum();
	}

	if (write(m_basic_fds2[m_appid], s.c_str(), s.size()) == -1) {
		errlog(string("[write_basic_log]write(): ") + strerror(errno));
	        close(m_basic_fds2[m_appid]);
		m_basic_fds2[m_appid] = -1;
		m_chksum1 = m_chksum2 = calc_checksum();
...
//		if(m_isgame)
//		{//游戏后台络的日志
//			oss << m_path2 << "/inbox/" << m_appid << "_game_basic_" << m_basic_ts2;
//		}
//		else
//		{//平台组络的日志
...
//}
//
//void StatLogger::write_custom_log2(const std::string& s, time_t ts)
//{
//	if(m_inited == 0)
//	{
//		errlog(string("object not inited"));
...
//
//	// 每20秒时间切换文件，跨天也切换文件
//	if ((ts < m_custom_ts2) || ((ts - m_custom_ts2) > 19) || (m_custom_fd2 == -1) ||  ((m_custom_ts2 + 8*60*60)/86400 != (ts + 8*60*60)/86400)) {
//		close(m_custom_fd2);
//		m_custom_ts2 = ts - (ts % 20);
//		ostringstream oss;
//		if(m_isgame)
//		{
//			oss << m_path2 << "/inbox/" << m_appid << "_game_custom_" << m_custom_ts2;
//		}
//		else
//		{
//			oss << m_path2 << "/inbox/" << m_appid << "_account_custom_" << m_custom_ts2;
//		}
//
