...
public:
     ...
     */
	StatLogger(int game_id, int32_t zone_id = -1, int32_t svr_id = -1, int32_t site_id = -1, int isgame = 1, bool writeToTongji=false, bool writeToTms=false);
	~StatLogger();

	StatLogger();
	void init(int game_id, int32_t zone_id = -1, int32_t svr_id = -1, int32_t site_id = -1, int isgame = 1);

	StatLogger* set(int game_id, int32_t zone_id, int32_t svr_id, int32_t site_id, int isgame,bool writeToTongji,bool writeToTms);
	/**
...
//	int32_t m_serverID;      // 应用/游戏的服务器ID，由游戏自定义，在统计平台可以设定ID对应的名称
...
//	int m_basic_fd2;       // 用于写入基础统计项的fd，5秒钟切换一个文件
	std::map<int,int> m_basic_fds1;       // 用于映射gameid到各游戏统计平台基础统计项的fd，5秒钟切换一个文件
	std::map<int,int> m_basic_fds2;       // 用于映射gameid到各游戏tms平台基础统计项的fd，5秒钟切换一个文件
...
//	time_t m_basic_ts2;    // 记录基础统计项文件的创建时间，用于判断5秒钟切换一个文件
	std::map<int,time_t> m_basic_tss1;    // 用于映射gameid到每个游戏统计平台文件的创建时间，用于判断5秒钟切换一个文件
	std::map<int,time_t> m_basic_tss2;    // 用于映射gameid到每个游戏tms平台文件的创建时间，用于判断5秒钟切换一个文件
